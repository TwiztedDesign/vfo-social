angular.module('app',[])
    ...
    .controller('ctrl', ['$scope','$http', function($scope, $http) {
        ...
        $scope.addEng = function(){
            let newEng = Object.assign({},activity);
            newEng.id=(new Date()).getTime();
            $scope.data.activities.items.unshift(newEng);
        };


        //HOW DO I GET MY ENGAGEMENTS ?
        vff.onController('data',(e)=>{
            $scope.data.activities = e.data;
            $scope.$apply();
        }, {changeOnly : false});

        const apiKey = 'SJlBfAtVKUrJZBfAt4tUSkGrMAF4F8';
        ...
        $scope.activateEngagement = function(item){
            ...
            let data =  Object.assign(item, {disabled : false,visible: true, start_time : Date.now()});
            let event = {
                // channel     : "controller",
                channel     : item.id+ "",
            ...
            setTimeout(() => {
                ...
                data = Object.assign(item, {
                    disabled : true,
                    answers: item.answers.map(function(a){
                        a.result = Math.random()*100;
                        return a;
                    })
                });
                $http.post(eventUrl, {event, initiator : "controller"});
        ...
        $scope.hideAllEng= function(){
            //TODO
            // remove all events from the overlay, Not from the controller.
            // Set all item.visiblity in controller to false
            $http.post(eventUrl, {event, initiator : "controller"});
        }
    }]);
